...
const normalizeMergeRefs = (merges: unknown): string[] => {
    if (!merges) return [];
    if (Array.isArray(merges)) {
        return merges
            .map(merge => String(merge))
            .filter(Boolean);
    }
    if (typeof merges === 'object') {
        const map = merges as Record<string, unknown>;
        const values = Object.values(map);
        const refs = values.every(value => typeof value === 'string')
            ? values
            : Object.keys(map);
        return refs
            .map(merge => String(merge))
...
const sheetJsMergesToRefs = (merges?: XLSX.Range[]): string[] => {
    if (!merges?.length) return [];
    return merges.map(merge => XLSX.utils.encode_range(merge));
};

function getRowOrNew(rows: RowMap, ri: number) {
    const existing = rows[ri];
    if (existing && typeof existing === 'object' && 'cells' in existing) {
        return existing;
    }
    const row = { cells: {} as Record<number, CellData> };
    rows[ri] = row;
    return row;
}

function applyMergeSpans(rows: RowMap, mergeRefs: string[]) {
    let maxRow = 0;
    let maxCols = 0;
    for (const mergeRef of mergeRefs) {
        ...
        const rn = range.e.r - range.s.r;
        const cn = range.e.c - range.s.c;
        if (rn <= 0 && cn <= 0) continue;
        maxRow = Math.max(maxRow, range.e.r + 1);
        maxCols = Math.max(maxCols, range.e.c + 1);

        const originRow = getRowOrNew(rows, range.s.r);
        const originCell = originRow.cells[range.s.c] ?? { text: '' };
        originCell.merge = [rn, cn];
        originRow.cells[range.s.c] = originCell;
        ...
        for (let ri = range.s.r; ri <= range.e.r; ri += 1) {
            ...
            for (let ci = range.s.c; ci <= range.e.c; ci += 1) {
                if (ri === range.s.r && ci === range.s.c) continue;
                delete row.cells[ci];
            }
        }
    }
    return { maxRow, maxCols };
}
...
const convertExcelJsWorksheet = (worksheet: ExcelJS.Worksheet, workbook: ExcelJS.Workbook): Pick<SheetData, 'rows' | 'cols' | 'styles' | 'merges' | 'freeze' | 'autofilter' | 'hyperlinks' | 'validations' | 'sheetProtection' | 'images' | 'backgroundImage'> => {
    ...
    const merges = normalizeMergeRefs(worksheet.model.merges);
    const mergeExtent = applyMergeSpans(rows, merges);
    maxRow = Math.max(maxRow, mergeExtent.maxRow);
    const colCount = Math.max(maxCols, worksheet.columnCount || 0, mergeExtent.maxCols);
...
const convertSheetJsWorksheet = (worksheet: XLSX.WorkSheet): Pick<SheetData, 'rows' | 'cols' | 'merges'> => {
    ...
    const merges = sheetJsMergesToRefs(worksheet['!merges']);
    const mergeExtent = applyMergeSpans(rows, merges);
    maxRow = Math.max(maxRow, mergeExtent.maxRow);
    const colCount = Math.max(maxCols, range.e.c - range.s.c + 1, mergeExtent.maxCols);
