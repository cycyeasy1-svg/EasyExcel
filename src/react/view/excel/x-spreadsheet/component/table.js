...
function computeAutoRowHeights(data, ctx) {
  ...
  const padding = cellPaddingWidth * zoom;
  ...
  rows.each((ri, row) => {
    ...
    Object.keys(row.cells).forEach((ci) => {
      ...
      const innerWidth = colWidth - padding * 2 - 2;
      ...
      const fontSizePx = getFontSizePxByPt(fontSizePt) * zoom;
      ...
      const neededHeight = lineCount * (fontSizePx + 2 * zoom) + padding * 2 + 2;
...
function tableFixedHeaderStyle(data) {
  const zoom = data?.zoom || 1;
  return {
    textAlign: 'center',
    textBaseline: 'middle',
    font: `500 ${npx(12 * zoom)}px Source Sans Pro`,
...
function getDrawBox(data, rindex, cindex, yoffset = 0) {
  ...
  return new DrawBox(left, top + yoffset, width, height, cellPaddingWidth * (data.zoom || 1));
}
/*
...
export function renderCell(draw, data, rindex, cindex, yoffset = 0) {
  ...
  draw.rect(dbox, () => {
    ...
    font.size = getFontSizePxByPt(font.size || (defaultStyle.font && defaultStyle.font.size) || 11) * (data.zoom || 1);
...
function renderFixedHeaders(type, viewRange, w, h, tx, ty) {
  ...
  // draw text
  // text font, align...
  draw.attr(tableFixedHeaderStyle(data));
  // y-header-text
