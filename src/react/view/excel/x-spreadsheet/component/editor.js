...
function applyVerticalCenter(editor, fontSizePt) {
  ...
  const sizePx = getFontSizePxByPt(fontSizePt ?? 11) * zoom;
  const lineH = sizePx + 2 * zoom;
...
function buildEditorFontCss(font, defaultFontName = 'Arial', zoom = 1) {
  const sizePx = getFontSizePxByPt(font?.size ?? 11) * zoom;
  const parts = [];
...
export default class Editor {
  ...
  setZoom(zoom) {
    const n = Number(zoom);
    this.zoom = Number.isFinite(n) && n > 0 ? n : 1;
  }
  ...
  applyCellStyle(cellStyle) {
    ...
    if (font) {
      const fontCss = buildEditorFontCss(font, 'Arial', this.zoom);
      const lineHeight = `${getFontSizePxByPt(font.size ?? 11) * this.zoom + 2 * this.zoom}px`;
